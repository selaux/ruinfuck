        ...
        code.into_iter().fold(AnalysisResults::default(), move |mut memo, v| {
...

            {
                let entry = match v {
                    ...
                    Node::Inc(_) => memo.nodes.entry(Node::Inc(0)),
                    Node::Dec(_) => memo.nodes.entry(Node::Dec(0)),
                    Node::Assign(_) => memo.nodes.entry(Node::Assign(0)),
...
mod tests {
    ...
    fn it_should_return_results_for_multiple_different_nodes() {
        ...
        expected_nodes.insert(Node::Inc(0), 1);
